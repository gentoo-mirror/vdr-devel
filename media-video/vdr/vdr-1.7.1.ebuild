...
APIWRAPPER=vdr-1.7.1-dvb-api-wrapper-0.4.diff
...
SRC_URI="ftp://ftp.cadsoft.de/vdr/Developer/${MY_P}.tar.bz2
	...
	dvbcompat? ( http://www.udo-richter.de/vdr/files/${APIWRAPPER} )"
...
src_unpack() {
	...
	if [[ ${api_minor:-0} -lt 3 ]]; then
		...
		if ! use dvbcompat; then
			eerror "You cannot compile VDR against old dvb-headers without USE=dvbcompat"
