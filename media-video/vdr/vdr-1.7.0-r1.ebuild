...
EXT_PATCH_FLAGS="analogtv atsc cmdsubmenu cutterlimit cutterqueue cuttime ddepgentry
	...
	rotor settime setup sortrecords sourcecaps livebuffer softosd cmdreccmdi18n
...
IUSE="debug vanilla dxr3 +h264 +s2api ehd ${EXT_PATCH_FLAGS} ${EXT_PATCH_FLAGS_RENAMED}"
...
check_menu_flags() {
	...
	if use s2api && ! use h264; then
		echo
		error "Please enable for s2api support also USE-Flag h264"
		echo
		die "s2api need also h264 USE-Flag"
	fi
}
...
src_unpack() {
	...
	if [[ ${api_version:-0} -lt 5 ]]; then
		...
		if use s2api; then
			eerror "You cannot compile VDR against old dvb-headers with USE=s2api"
			die "DVB headers too old, disable USE=s2api"
		fi
	else
	...
	if ! use vanilla; then
		...
		if use h264; then
			epatch "${EXT_DIR}/extras/vdr-1.7.0-ext_h264.diff"

			if use s2api; then
				epatch "${EXT_DIR}/extras/vdr-1.7.0-ext-h264_s2.diff"
			fi
		fi
		...
		if use ehd; then
			einfo "Applying Reel eHD extension patch"
			epatch "${FILESDIR}"/"${P}"_ehd-svn-9336.diff
		fi
